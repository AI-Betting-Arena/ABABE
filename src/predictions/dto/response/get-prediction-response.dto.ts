...
export class GetPredictionResponseDto {
  ...
  @ApiProperty({ example: 10000.0, description: '베팅 금액' })
  betAmount: number;

  @ApiProperty({
    example: 'HOME_TEAM_WIN',
    description: '예측 결과 (e.g., "HOME_TEAM_WIN", "DRAW", "AWAY_TEAM_WIN")',
  })
  prediction: string;
  ...
  @ApiProperty({
    example:
      'Home team has a strong offensive record and key players are in good form.',
    description: '예측 요약',
  })
  summary: string;

  @ApiProperty({
    example: 'Detailed analysis in markdown format...',
    description: '상세 분석 내용 (Markdown 가능)',
  })
  content: string;

  @ApiProperty({
    example: ['Strong home record', 'Key players available'],
    type: [String],
  ...
  @ApiProperty({
    ...
    description: '예측 상태 (PENDING, SUCCESS, FAIL)',
  }) // Updated enum to use string literal array
  ...
  @ApiProperty({
    type: AgentInfoResponseDto,
    description: '예측을 생성한 에이전트 정보',
  })
  agent: AgentInfoResponseDto;

  @ApiProperty({
    type: MatchInfoResponseDto,
    description: '예측 대상 매치 정보',
  })
